...
class Scorer:
    ...
    def compute_class_probabilities(self, labels: ArrayLike) -> Dict[Any, float]:
        ...
        denom = total_count + alpha * num_classes
        
        for label in self.class_labels:
            class_count = np.sum(labels == label)
            class_probabilities[label] = (class_count + alpha) / denom
        # <<< END OF YOUR CODE <<<
    ...
    def subset_data(self, data: ArrayLike, labels: ArrayLike, split_attribute: int, split_value: Any) -> Tuple[ArrayLike, ArrayLike]:
        ...
        # >>> YOUR CODE HERE >>>
        data_subset = np.array(data)
        labels_subset = np.array(labels)

        split_indices = np.where(data[:, split_attribute] == split_value)[0]

        data_subset = data[split_indices]
        labels_subset = labels[split_indices]
        # <<< END OF YOUR CODE <<<
    ...
    def information_gain(self, data: ArrayLike, labels: ArrayLike, split_attribute: int) -> float:
        ...
        for value in unique_values:
            # Subset the data and labels for instances where the split attribute has this value
            data_subset, labels_subset = self.subset_data(data, labels, split_attribute, value)
            
            # Compute the weighted entropy for this subset
