...
class Leaf(Node):
    ...
    def predict_class_probabilities(self, X: ArrayLike) -> np.ndarray:
        ...
        """
        # >>> YOUR CODE HERE >>>
        probabilities = np.array([[self.class_probabilities[label] for label in self.class_labels]])
        # <<< END OF YOUR CODE <<<
    ...
    def predict(self, X: ArrayLike) -> np.ndarray:
        ...
        predicted_class = self.class_labels[max_prob_index]
        labels = np.full(len(X), predicted_class)
        # <<< END OF YOUR CODE <<<
        return labels
...
class DecisionTree:
    ...
    def _build_tree(self, X: ArrayLike, y: ArrayLike, max_depth: int, exclude: set = set()) -> Node:
        ...
        if 'NA' not in children:
            # >>> YOUR CODE HERE >>>
            class_probabilities = self.scorer.compute_class_probabilities([])
